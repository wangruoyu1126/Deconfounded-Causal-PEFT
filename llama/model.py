...
class Attention(nn.Module):
    ...
    def forward_with_attn(
        ...
        return self.wo(output), scores, adapter_scores
...
class TransformerBlock(nn.Module):
    ...
    def forward_with_attn(
        ...
        h, attn, adapter_attn = self.attention.forward_with_attn(self.attention_norm(x), start_pos, freqs_cis, mask, adapter)
        h = x + h
        out = h + self.feed_forward.forward(self.ffn_norm(h))
        return out, attn, adapter_attn
...
class Transformer(nn.Module):
    ...
    @torch.inference_mode()
    def forward_with_attn(self, tokens: torch.Tensor, start_pos: int):
        _bsz, seqlen = tokens.shape
        h = self.tok_embeddings(tokens)
        ...
        for layer in self.layers[: -1 * self.params.adapter_layer]:
            # h = layer(h, start_pos, freqs_cis, mask)
            h, attn, adapter_attn = layer.forward_with_attn(h, start_pos, freqs_cis, mask)
            attns.append(attn)
            adapter_attns.append(adapter_attn)

        # for adapter layers
        layer_index = 0
        for layer in self.layers[-1 * self.params.adapter_layer :]:
            h, attn, adapter_attn = layer.forward_with_attn(h, start_pos, freqs_cis, mask, prompt[layer_index])
            attns.append(attn)
            adapter_attns.append(adapter_attn)
            layer_index = layer_index + 1
        h = self.norm(h)
        ...
        return output.float(), attns, adapter_attns
